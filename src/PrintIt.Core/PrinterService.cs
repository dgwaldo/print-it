...
namespace PrintIt.Core {
    internal sealed class PrinterService : IPrinterService {
        ...
        public void RemovePrinter(string printerPath) {
            if (string.IsNullOrEmpty(printerPath))
                throw new ArgumentException("Path cannot be null or empty", nameof(printerPath));

            string cmdPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.System), "rundll32.exe");
            CommandExecutionResult executionResult = _commandService.Execute(cmdPath, "printui.dll,PrintUIEntry", "/q", "/gd", "/dn", $"/n\"{printerPath}\"");

            // ReSharper disable once InvertIf
            if (!executionResult.Success) {
                var message = string.Join(Environment.NewLine, executionResult.Output);
                throw new RemovePrinterFailedException(printerPath, message);
            }
        }
    ...
    public interface IPrinterService {
        ...
        void RemovePrinter(string printerPath);
    }
    ...
    public sealed class RemovePrinterFailedException : Exception {
        public RemovePrinterFailedException(string printerPath, string message)
            : base($"Failed to remove printer '{printerPath}' with message: {message}") {
            PrinterPath = printerPath;
        }
